...
public class CreateRandomSpecificationByGraph extends GraphUtility implements Serializable {
...
/** Trivial constructor. */
	public CreateRandomSpecificationByGraph() {
		super(KeyEvent.VK_G,"Create Random Graph");
	}

/** Trivial constructor. */
	public CreateRandomSpecificationByGraph(int key, String s) {
		super(key,s);
	}
		...
		for(Node n1 : g.getNodes()) {
			for(Node n2 : g.getNodes()) {
				...
				if(rand < attractionChance) {
					...
					g.addEdge(e);
					e.setLabel(AreaSpecification.findAttractorLabel(e));
				} else {
					Edge e = new Edge(n1,n2,APCircleDisplay.REPULSOR);
					g.addEdge(e);
					e.setLabel(AreaSpecification.findRepulsorLabel(e));
				}
			}
		}		
