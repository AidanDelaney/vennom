...
import org.eulerdiagrams.vennom.graph.ItemType;
...
public class EditEdgeTypeDialog extends JDialog implements ActionListener {
	...
	public EditEdgeTypeDialog(ManageEdgeTypesDialog manageDialog, APCirclePanel graphPanel, EdgeType edgeType, boolean inNewFlag, String windowLabel) {

		super(graphPanel.getContainerFrame(),windowLabel,true);
...
// parent
	...
	public void okButton(ActionEvent event) {
		...
		if(newFlag) {
			...
			if(label.equals("")) {
				JOptionPane.showMessageDialog(gp, "Cannot have an empty label.","Error",JOptionPane.PLAIN_MESSAGE);
				return;
			}
			if(EdgeType.withLabel(label) != null) {
				JOptionPane.showMessageDialog(gp, "That label already exists.","Error",JOptionPane.PLAIN_MESSAGE);
				return;
			}
		...
		if(!et.setParent(parent)) {
			JOptionPane.showMessageDialog(gp, "Invalid Parent, loop caused in parent heirarchy.","Error",JOptionPane.PLAIN_MESSAGE);
			return;
		}
